...
class KeyTeleop():
    ...
    def __init__(self, interface):
        ...
        forward_min = rospy.get_param('~turbo/linear_forward_min', 0)
        forward_max = rospy.get_param('~turbo/linear_forward_max', 255)
        self._forward = Velocity(forward_min, forward_max, self._num_steps)

        backward_min = rospy.get_param('~turbo/linear_backward_min', 0)
        backward_max = rospy.get_param('~turbo/linear_backward_max', 255)
        self._backward = Velocity(backward_min, backward_max, self._num_steps)

        angular_min = rospy.get_param('~turbo/angular_min', 0)
        angular_max = rospy.get_param('~turbo/angular_max', 255)
...
class SimpleKeyTeleop():
    ...
    def __init__(self, interface):
        ...
        self._forward_rate = rospy.get_param('~forward_rate', 100)
        self._backward_rate = rospy.get_param('~backward_rate', 100)
        self._rotation_rate = rospy.get_param('~rotation_rate', 100)
        self._last_pressed = {}
        self._angular = 0
        self._linear = 0
    ...
    def _set_velocity(self):
        ...
        for a in self._last_pressed:
            if now - self._last_pressed[a] < 0.7:
                keys.append(a)
        linear = 0.0
        angular = 0.0
        for k in keys:
    ...
    def _publish(self):
        self._interface.clear()
        self._interface.write_line(2, 'Linear: %f, Angular: %f hz: %d' %
                                   (self._linear, self._angular, self._hz))
